...
double comparePoses(struct skeletonHuman * pose1 , struct skeletonHuman * pose2,struct skeletonJointsWeCareAbout * whatJointsToUse)
{
  double score=0.0;
  unsigned int i=0,okToUse=1,jointsUsed=0;
  for (i=0; i<HUMAN_SKELETON_PARTS; i++)
  {
    if (whatJointsToUse!=0)
    {
      if (whatJointsToUse->joint[i])
                                     { okToUse=1; }
                               else
                                     { okToUse=0; }
    }
  ...
  //Normalize score according to joints we are actually used
  if (jointsUsed!=0)
     { score=score/jointsUsed; }
...
{
  if (currentRememberedSkeletons==0)  { return 0; }
