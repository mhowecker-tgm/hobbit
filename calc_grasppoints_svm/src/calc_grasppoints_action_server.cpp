 ...
 *
 *     Modified calc_grasppoints.cpp on Dec 03, 2014 to have it operated as ActionServer
 *     Author: David Fischinger, Vienna University of Technology
 *
 *     This ActionServer is calculating grasp points and approach vectors, given a point cloud as input/goal.
 *     In a first step a heightsgrid is created from the point cloud
 *     (for rotated and tilted pointclouds <==> different rolls of hand and approaching values).
 *     For each 14x14 square of the heightgrid a featurevector is created.
...
//Actionlib
#include <actionlib/server/simple_action_server.h>			//new 3.12.2014
#include <hobbit_msgs/CalcGraspPointsServerAction.h>		//new 3.12.2014
...
protected:
	ros::NodeHandle nh_;
	actionlib::SimpleActionServer<hobbit_msgs::CalcGraspPointsServerAction> as_;
	std::string action_name_;
	// create messages that are used to published feedback/result
	hobbit_msgs::CalcGraspPointsServerFeedback feedback_;
	hobbit_msgs::CalcGraspPointsServerResult result_;
...
public:
	...
	string gp_result; //saves return values for ActionServer (bad programming style)
 	...
 	//void read_pc_cb(const sensor_msgs::PointCloud2ConstPtr& pc_in); //before actionserver
 	void read_pc_cb(const hobbit_msgs::CalcGraspPointsServerGoalConstPtr &goal); //new 3.12.2014
