#include "../include/new_layers/nav_layer_global3.h"
 #include<costmap_2d/costmap_math.h>
 
 #include <pluginlib/class_list_macros.h>
 PLUGINLIB_EXPORT_CLASS(costmap_2d::NavLayerGlobal3, costmap_2d::Layer)
 ...
 {
 void NavLayerGlobal3::onInitialize()
 {
   ...
   while (ss >> source)
   {
     ...
     if (data_type == "LaserScan")
     {
       ...
       if (inf_is_valid)
       {
         filter->registerCallback(
             boost::bind(&NavLayerGlobal3::laserScanValidInfCallback, this, _1, observation_buffers_.back()));
       }
       else
       {
         filter->registerCallback(
             boost::bind(&NavLayerGlobal3::laserScanCallback, this, _1, observation_buffers_.back()));
       }
     ...
     else if (data_type == "PointCloud")
     {
       ...
       filter->registerCallback(
           boost::bind(&NavLayerGlobal3::pointCloudCallback, this, _1, observation_buffers_.back()));
     ...
     else
     {
       ...
       filter->registerCallback(
           boost::bind(&NavLayerGlobal3::pointCloud2Callback, this, _1, observation_buffers_.back()));
 ...
 void NavLayerGlobal3::setupDynamicReconfigure(ros::NodeHandle& nh)
 {
   ...
   dynamic_reconfigure::Server<costmap_2d::ObstaclePluginConfig>::CallbackType cb = boost::bind(
       &NavLayerGlobal3::reconfigureCB, this, _1, _2);
   dsrv_->setCallback(cb);
 }
 
 void NavLayerGlobal3::reconfigureCB(costmap_2d::ObstaclePluginConfig &config, uint32_t level)
 {
 ...
 void NavLayerGlobal3::laserScanCallback(const sensor_msgs::LaserScanConstPtr& message,
 ...
 void NavLayerGlobal3::laserScanValidInfCallback(const sensor_msgs::LaserScanConstPtr& raw_message, 
                                               const boost::shared_ptr<ObservationBuffer>& buffer){
 ...
 void NavLayerGlobal3::pointCloudCallback(const sensor_msgs::PointCloudConstPtr& message,
                                                const boost::shared_ptr<ObservationBuffer>& buffer)
 {
 ...
 void NavLayerGlobal3::pointCloud2Callback(const sensor_msgs::PointCloud2ConstPtr& message,
                                                 const boost::shared_ptr<ObservationBuffer>& buffer)
 {
   //buffer the point cloud
   buffer->lock();
 ...
 void NavLayerGlobal3::addStaticObservation(costmap_2d::Observation& obs, bool marking, bool clearing)
 {
   if(marking)
 ...
 bool NavLayerGlobal3::getMarkingObservations(std::vector<Observation>& marking_observations) const
 {
   bool current = true;
   //get the marking observations
 ...
 bool NavLayerGlobal3::getClearingObservations(std::vector<Observation>& clearing_observations) const
 {
   bool current = true;
   //get the clearing observations
 ...
 void NavLayerGlobal3::raytraceFreespace(const Observation& clearing_observation, double* min_x, double* min_y,
 ...
 void NavLayerGlobal3::deactivate()
 {
   for (unsigned int i = 0; i < observation_subscribers_.size(); ++i)
   {
     if (observation_subscribers_[i] != NULL)
 ...
 void NavLayerGlobal3::updateRaytraceBounds(double ox, double oy, double wx, double wy, double range, double* min_x, double* min_y,
                                          double* max_x, double* max_y)
 {
 ...
 void NavLayerGlobal3::onFootprintChanged()
 {
