...
int isSkeletonStanding(struct fallState * fs)
{
 // if (isSkeletonTooOld(fs,frameTimestamp)) { return 0; }
  ...
  if (fs->headLookingDirection==HEAD_LOOKING_CENTER)      { height=250; } else
  if (fs->headLookingDirection==HEAD_LOOKING_LITTLE_DOWN) { height=190;  } else
...
int isSkeletonFallenAndFar(struct fallState * fs)
{
  ...
  if (fs->headLookingDirection==HEAD_LOOKING_CENTER)      { height=380; } else
  if (fs->headLookingDirection==HEAD_LOOKING_LITTLE_DOWN) { height=270; } else
  ...
  unsigned int closePoints = 0;
   ...
   for (i=0; i<fs->numberOfJoints; i++)
   {
    ...
    {
      if ((fs->currentJoint3D[i].z!=0) && (fs->currentJoint3D[i].z < closeBorderZ))      { ++closePoints;  } else
      if (fs->currentJoint3D[i].z > farBorderZ)        { ++farPoints;    }
      //-------
      if ( (fs->currentJoint2D[i].y!=0)&& (fs->currentJoint2D[i].y>=height) )    { ++lowPoints;  } else
      if ( (fs->currentJoint2D[i].y!=0)&& (fs->currentJoint2D[i].y<height) )     { ++highPoints; }
    }
   }
