...
unsigned int temperatureSensorSensesHuman(float tempDetected , unsigned int tempTimestamp , unsigned int frameTimestamp)
{
 unsigned int temperatureFrameOffset = ABSDIFF(frameTimestamp,tempTimestamp);
  if (
    ...
    {
        //fprintf(stderr,"HUMAN TEMP %0.2f <= %0.2f <= %0.2f , sensor lag %u \n",minimums.objectTemperature , tempDetected , maximums.objectTemperature , temperatureFrameOffset);
        return 1;
    }
   //fprintf(stderr,"NONHUMAN TEMP %0.2f <!= %0.2f <!= %0.2f , sensor lag %u\n",minimums.objectTemperature , tempDetected , maximums.objectTemperature , temperatureFrameOffset);
   return 0;
}
