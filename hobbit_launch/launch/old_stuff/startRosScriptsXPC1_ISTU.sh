...
echo "Running mobile platform"
screen -S 'Mobile Platform' -d -m bash -c '~/ScreenScripts/mobilePlatform.sh;bash'
sleep $MEDIUMPAUSE

echo "Running go to pose"
screen -S 'Go To Pose' -d -m bash -c '~/ScreenScripts/goToPose.sh;bash'
sleep $MEDIUMPAUSE
